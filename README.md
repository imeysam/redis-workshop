...
این دستورات تا زمانی که داده‌ای برای برداشتن وجود نداشته باشد می‌توانند Client را Block کنند و منتظر می‌مانند تا داده جدید وارد لیست شود.
...
فرض کنید ظرفیت یک Event برابر با 1 باشد:

```text
capacity = 1
```

 و دو User همزمان می‌خواهند ثبت‌نام کنند.

بدون مکانیزم‌های کنترل همزمانی (Concurrency)، ممکن است هر دو User مقدار یکسانی را ببینند و هر دو فکر کنند ظرفیت وجود دارد.
