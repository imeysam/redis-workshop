...
این دستورات تا زمانی که داده‌ای برای برداشتن وجود نداشته باشد می‌توانند Client را Block کنند.
...
در Terminal دوم با استفاده از این دستور داده‌ها را به ترتیب دریافت می‌کند

```bash
BLPOP jobs 0
```
پارامتر 0 یعنی بدون Timeout و تا زمان رسیدن داده منتظر بمان.
